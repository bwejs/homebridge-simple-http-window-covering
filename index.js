...
module.exports = function(homebridge) {
  ...
  homebridge.registerAccessory("homebrige-simple-http-window-covering", "HTTP-Window-Covering", HTTPWindowCoveringAccessory);
}

function HTTPWindowCoveringAccessory(log, config) {

  this.log = log;
  this.name = config["name"];
  this.openURL = config["open-URL"];
  this.closeURL = config["close-URL"];
  this.valueMapping = config['valueMapping'];
  this.mode = Characteristic.PositionState.STOPPED;
  this.openTime = config["open-Time"] 	  	 	|| 29.0;;
  this.closeTime = config["close-Time"] 	  	 	|| 24.0;;
  this.current = 100;
  this.target = 100;
  this.counter = 0;
...
HTTPWindowCoveringAccessory.prototype.sendSignalOpen = function(open) {
  if (open){
    this.log("Will Send on OpenURL");
  } else {
    this.log("Will Send on CloseURL");
  }
  var url = open ? this.openURL : this.closeURL;
  request.put({
    url: url,
    qs: { }
